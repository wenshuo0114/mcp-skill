...
def disk_options() -> list[Option]:
    return [
        Option(1, "✅ 授权只读扫描整盘", "扫整盘，不含其他用户的目录（默认）",
               关键字=["授权", "整盘", "扫", "扫描", "只读", "继续", "可以", "好"], 区分词=["不含", "不包含", "只我", "只扫我", "跳过其他", "跳过别人"], 值=False),
        Option(2, "✅ 授权只读扫描整盘，含其他用户目录", "扫整盘，并包含其他用户的目录（只在这台机器完全属于你、或你有管理职责时）",
               关键字=["授权", "整盘", "扫", "扫描", "只读", "继续", "可以", "好"], 区分词=["含其他", "包含其他", "其他用户", "全部用户", "所有用户", "别的用户"], 值=True),
        Option(3, "不扫", "不扫整盘。可以改用 mode=文件夹 只扫你自己的目录",
               关键字=STOP_WORDS, 否定优先=True, 值=None),
    ]
...
def neutralize_options(filename: str, line_no: int | None, whole_file: bool) -> list[Option]:
    ...
    return [
        ...
        Option(3, "先出重写方案", "改动大：先在单独会话/子代理里产出重写方案，看完再决定",
               关键字=["重写", "方案", "另开", "先看"], 区分词=["重写", "方案", "另开"], 值="rewrite_plan"),
    ]
