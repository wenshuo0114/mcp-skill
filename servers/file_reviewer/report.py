...
class Report:
    ...
    def render(self) -> str:
        ...
        pers = d.get("持久化清单") or {}
        if pers:
            w(f"## 持久化位置清单（{pers.get('记录时间')}；只列文件，运行态请自跑官方命令）")
            w("")
            w("| 类别 | 路径 | 说明 | 触发时机 | 可达 | 文件数 | 不可达原因 |")
            w("|---|---|---|---|---|---|---|")
            for l in pers.get("位置", []):
                w("| " + " | ".join(_cell(l.get(k)) for k in ("类别", "路径", "说明", "触发时机", "可达", "文件数", "不可达原因")) + " |")
            w("")
            w(f"- 扫描文件数：{pers.get('扫描文件数')}；本清单产生的发现数：{pers.get('发现数')}（已并入发现清单）")
            guide = pers.get("凭据轮换根治法") or {}
            if guide:
                w("- 凭据轮换根治法（机器已售出 / 不再受控 / 文件读不到时唯一合规的根治）：")
                w(f"  - 原则：{guide.get('原则')}")
                out.extend(f"  - {x}" for x in guide.get("步骤", []))
                w(f"  - 不要做的：{guide.get('不要做的')}")
            w("")
        notes = d.get("路径说明") or []
        if notes:
