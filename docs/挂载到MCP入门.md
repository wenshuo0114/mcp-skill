...
**隐私一句话**（详细见 [隐私边界.md](隐私边界.md)）：审查器只读你给出的、且属于你的路径；**永不进其他用户家目录**；没有「含其他用户」入口；mcp.json 的 `command` 只决定用哪个 Python **启动**审查器，不是授权去读谁的盘。

---

## 〇、mcp.json 里的「指向」到底指什么（避免误会）

很多人看到 `"command": "…/.venv/bin/python"` 会以为「这是去读 .venv 里的文件」或「这是去读别人」。不是。

| 字段 | 指向的东西 | 用人话讲 |
|---|---|---|
| `command` | 本机上的一个 **Python 可执行文件**（建议是你刚建的 `.venv` 里那个） | 「用这把干净的刀来干活」 |
| `args` | 固定 `-m servers.file_reviewer.server` | 「干的活 = 启动本仓库的审查器模块」 |
| `cwd` | mcp-skill 仓库根目录 | 「站在这个目录里找模块」，不是「默认审这个」 |
| `env` 报告目录 | 你的 `~/.mcp-skill/reports` | 「报告写在你家目录，别写进网站仓」 |

被审哪个文件夹，要另说：对话里 `review_open("/你自己的路径/某项目")`。  
别人的 `/home/别人/...` → 工具会拒绝。
